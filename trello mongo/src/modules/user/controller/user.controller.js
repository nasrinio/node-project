...
export const signUp = async (req, res, next) => {
    try {
        ...
        if (password != cPassword) return res.json({ message: "password mis match cpassword" })
...
export const login = async (req, res, next) => {
    try {
        ...
        const token = jwt.sign({ _id: user._id, email: user.email }, 'trello')
...
export const changePassword = async (req, res, next) => {
    try {
        ...
        if (newPassword != cPassword) return res.json({ message: "newPassword mis match cpassword" })
        const match = bcrypt.compareSync(oldPassword, req.user.password)
        console.log(match);
        if (!match) return res.json({ message: "old password not match your password" })
